...
<div class="transfer-section">
  <c:if test = "${ param.USECOUNT == 0 }">
    <h5>Delete Activation Code Table - ${param.NAME}</h5>
  ...
  <c:if test = "${ param.USECOUNT != 0 }">
    ...
    <p style="margin-bottom: .2rem;">The ${param.NAME} table is currently used by ${param.USECOUNT} product<c:if test = "${ param.USECOUNT > 1 }">s</c:if>.</p>
    <p style="margin-bottom: .2rem;">In order to delete this table you must disassociate the product<c:if test = "${ param.USECOUNT > 1 }">s</c:if> from this table.</p>
